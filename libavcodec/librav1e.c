...
static int get_stats(AVCodecContext *avctx, int eos)
{
    ...
    data = rav1e_twopass_out(ctx->ctx);
    if (!data)
        return 0;

    if (!eos) {
        uint8_t *tmp = av_fast_realloc(ctx->pass_data, &ctx->pass_size,
                                      ctx->pass_pos + data->len);
        if (!tmp) {
            rav1e_data_unref(data);
            return AVERROR(ENOMEM);
        }

        ctx->pass_data = tmp;
        memcpy(ctx->pass_data + ctx->pass_pos, data->data, data->len);
        ctx->pass_pos += data->len;
    } else {
        size_t b64_size = AV_BASE64_SIZE(ctx->pass_pos);

        memcpy(ctx->pass_data, data->data, data->len);
...
static const AVOption options[] = {
    ...
    { "speed", "what speed preset to use", OFFSET(speed), AV_OPT_TYPE_INT, { .i64 = -1 }, -1, 9, VE },
